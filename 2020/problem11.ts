...
function isOccupiedSeat(input: string[][], i: number, j: number) {
  return isWithinBounds(input, i, j) && input[i][j] === '#';
}

function isEmptySeat(input: string[][], i: number, j: number) {
  return isWithinBounds(input, i, j) && input[i][j] === 'L';
}

function walkAround(x: number, y: number, onSeat: (i: number, j: number, iMod: number, jMod: number) => any) {
  for (let i = x - 1; i <= x + 1; i++) {
    for (let j = y - 1; j <= y + 1; j++) {
      if (!(i === x && j === y)) {
        if (onSeat(i, j, i - x, j - y) === -1) return;
      }
    }
  }
}

function numOccupiedAround(input: string[][], x: number, y: number) {
  let occupiedAround = 0;
  walkAround(x, y, (i, j) => (occupiedAround += isOccupiedSeat(input, i, j) ? 1 : 0));
  return occupiedAround;
}

function walkAroundAndOut(input: string[][], x: number, y: number, onSeat: (i: number, j: number) => any) {
  walkAround(x, y, (i, j, iMod, jMod) => {
    let mult = 0;
    let nextI = i;
    let nextJ = j;

    while (isWithinBounds(input, nextI, nextJ)) {
      if (onSeat(nextI, nextJ) === -1) break;
      mult++;
      nextI = i + iMod * mult;
      nextJ = j + jMod * mult;
    }
...
function numOccupiedAroundAndOut(input: string[][], x: number, y: number) {
  let occupiedAroundAndOut = 0;

  walkAroundAndOut(input, x, y, (i, j) => {
    if (isEmptySeat(input, i, j)) {
      return -1;
    } else if (isOccupiedSeat(input, i, j)) {
      occupiedAroundAndOut++;
      return -1;
    }
  });
  return occupiedAroundAndOut;
}

function clone2DArray(input: string[][]): string[][] {
...
function part2(input: string[][]) {
  ...
  let areSame = false;
  while (!areSame) {
    const modified = clone2DArray(last);
    let occupied = 0;
    areSame = true;
    // Loop through last map
    last.forEach((row, i) => {
      row.forEach((seat, j) => {
        if (seat === 'L' && numOccupiedAroundAndOut(last, i, j) === 0) modified[i][j] = '#';
        else if (seat === '#' && numOccupiedAroundAndOut(last, i, j) >= 5) modified[i][j] = 'L';

        areSame = areSame && seat === modified[i][j];

        if (areSame && seat === '#') occupied++;
      });
    });

    if (areSame) return occupied;
...
function part1(input: string[][]) {
  ...
  while (!areSame) {
    ...
    last.forEach((row, i) => {
      row.forEach((seat, j) => {
        if (seat === 'L' && numOccupiedAround(last, i, j) === 0) modified[i][j] = '#';
        else if (seat === '#' && numOccupiedAround(last, i, j) >= 4) modified[i][j] = 'L';
...
const providedInput = `LLLLL.LLLLLLLL..LLLLLLLLLLLLLL.LLLL..LL..LLLLLLLL.LLLL.LLLLLLLLLLLL.LLLLLL.LLLLLL.LLLLLLLL
...
const exampleInput1 = `L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
