...
function part1(inputs: string[]) {
  ...
  inputs.forEach((input) => {
    const { letter, password, min, max } = parseInput(input);
...
function part2(inputs: string[]) {
  ...
  inputs.forEach((input) => {
    ...
    const validAtMin = password[min - 1] === letter;
    const validAtMax = password[max - 1] === letter;

    if ((validAtMin && !validAtMax) || (!validAtMin && validAtMax)) {
      count++;
    }
  });
...
function parseInput(input: string) {
  const [policyAndLetter, password] = input.split(': ');
  const [counts, letter] = policyAndLetter.split(' ');
  const [minStr, maxStr] = counts.split('-');
...
8-11 k: kzkkpkkbkzq
4-7 h: hhxhzmwhhhh
11-16 r: rrrrrrrrrrrrrrrlr`.split('\n');
