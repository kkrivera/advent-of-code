...
function part1(input: string[][]) {
  return input.reduce((acc, forms) => {
    const uniqueCharacters = forms.reduce((set, form) => {
      for (let char of form) set.add(char);
      return set;
    }, new Set()).size;

    return acc + uniqueCharacters;
  }, 0);
}
  ...
  return input
    .map((forms) =>
      forms.map((form) => {
        ...
        for (let char of form) formSet.add(char);
        return formSet;
      })
    )
    .reduce((acc, forms) => {
      let charsFoundInAllForms = 0;
      const [firstForm] = forms;

      if (forms.length === 1) {
        // If length of forms is 1, then add the entire size/length of the form
        charsFoundInAllForms += firstForm.size;
      } else {
        // Iterate through the first form and verify each character that is present in other forms
        firstForm.forEach((answer) => {
          const charFoundInAllForms = forms.reduce(
            (acc, form) => acc && form.has(answer),
            true
          );

          if (charFoundInAllForms) charsFoundInAllForms++;
        });
      }

      return acc + charsFoundInAllForms;
    }, 0);
}
