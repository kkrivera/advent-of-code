...
interface ContainingBags {
  [container: string]: { [bagType: string]: number };
}

interface ParentBagsByChild {
  [childBagType: string]: Set<string>;
}

function removeBagStr(input: string) {
  return input.replace(' bags', '').replace(' bag', '');
}

function parseInput(input: string): ContainingBags {
  return input.split('.\n').reduce((acc, rule) => {
    const [container, contains] = rule.split(' contain ');
    const containedBags = contains
      .split(', ')
      .filter((bag) => !bag.startsWith('no other bags'))
      .reduce((acc, bag) => {
        const count = parseInt(bag.slice(0, 1));
        const bagType = removeBagStr(bag.slice(2));
        acc[bagType] = count;
        return acc;
      }, {});
    acc[removeBagStr(container)] = containedBags;
    return acc;
  }, {});
}

function getAllUniqueParents(
  childBagType: string,
  parentBagsByChild: ParentBagsByChild,
  parents: Set<string> = new Set()
) {
  const parentBags = parentBagsByChild[childBagType];
  parentBags?.forEach((parentBag) => {
    parents.add(parentBag);
    getAllUniqueParents(parentBag, parentBagsByChild, parents);
  });

  return parents;
}

function countChildren(parentBagType: string, parentBags: ContainingBags): number {
  const childBags = parentBags[parentBagType];
  if (!childBags) return 0;

  return Object.keys(childBags).reduce((acc, childBagType) => {
    const childBagCount = childBags[childBagType];
    return acc + childBagCount + childBagCount * countChildren(childBagType, parentBags);
  }, 0);
}

function part1(parentBags: ContainingBags) {
  const parentBagsByChild: ParentBagsByChild = Object.keys(parentBags).reduce((acc, parentBagType) => {
    const childBags = parentBags[parentBagType];

    Object.keys(childBags).forEach((childBagType) => {
      if (!acc[childBagType]) acc[childBagType] = new Set();
      acc[childBagType].add(parentBagType);
    });
    return acc;
  }, {} as { [childBagType: string]: Set<string> });

  return getAllUniqueParents('shiny gold', parentBagsByChild).size;
}

function part2(parentBags: ContainingBags) {
  return countChildren('shiny gold', parentBags);
}

const input = `light green bags contain 2 pale cyan bags.
...
run(part1, parsedInput);
run(part2, parsedInput);
