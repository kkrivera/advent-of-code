...
function part1(input: Input[]) {
  const ingredientsByAllergen = input.reduce((acc, food) => {
  ...
  while (
    ...
    const [ingredientToRemove] = ingredientsByAllergen[nextAllergen];

    Object.keys(ingredientsByAllergen)
  ...
  console.log(
    Object.keys(ingredientsByAllergen)
      ...
      .map((allergen) => ingredientsByAllergen[allergen])
...
const exampleInput1 = `mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)`;

const input: Input[] = providedInput.split('\n').reduce((acc, line) => {
  const allergens = line.match(/ \(contains (.+?)\)$/);

  acc.push({
    ingredients: line.replace(allergens[0], '').split(' '),
