...
const eyeColors: Set<EyeColor> = new Set(['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']);

function validPassports(passports: Passport[], verify: (passport: Passport, passportKey: PassportKey) => boolean) {
  return passports.reduce((acc, passport) => {
    const isValidPassport = passportKeys
      .filter((passportKey) => passportKey !== 'cid')
      .reduce((isValid, passportKey) => {
        return isValid && verify(passport, passportKey);
      }, true);

    return isValidPassport ? acc + 1 : acc;
  }, 0);
}

function part1(passports: Passport[]) {
  return validPassports(passports, (passport, key) => passport.hasOwnProperty(key));
}

function part2(passports: Passport[]) {
  return validPassports(passports, (passport, key) => {
    if (!passport.hasOwnProperty(key)) return false;

    const value = passport[key];
    let valid = false;

    if ((key === 'byr' || key === 'iyr' || key === 'eyr') && value.length === 4 && value.match(/^\d{4}$/)) {
      const year = parseInt(value);
      valid =
        (key === 'byr' && year >= 1920 && year <= 2002) ||
        (key === 'iyr' && year >= 2010 && year <= 2020) ||
        (key === 'eyr' && year >= 2020 && year <= 2030);
    } else if (key === 'hgt' && value.match(/^\d+(in|cm)$/)) {
      const num = parseInt(value.replace('cm', '').replace('in', ''));
      valid = (value.endsWith('cm') && num >= 150 && num <= 193) || (value.endsWith('in') && num >= 59 && num <= 76);
    } else if (key === 'hcl' && value.match(/^#[a-f0-9]{6}$/)) valid = true;
    else if (key === 'ecl' && eyeColors.has(value as EyeColor)) valid = true;
    else if (key === 'pid' && value.match(/^\d{9}$/)) valid = true;
    return valid;
  });
}

function processInput(input: string): Passport[] {
  const passportStr = input.split('\n\n');
  const passportBlocks: string[] = passportStr.reduce((acc, cur) => {
    acc.push(cur.split('\n').join(' '));
    return acc;
  }, []);
  const passportLines = passportBlocks.map((passportKeyValueStr) => passportKeyValueStr.split(' '));
  const passports = passportLines.map((passportSegments) =>
    passportSegments.reduce((acc, cur) => {
...
const input = `iyr:2010 ecl:gry hgt:181cm
...
const passportsInput = processInput(input);
run(part1, passportsInput);
run(part2, passportsInput);
